...
public class SyntaxAnalyzer {
    ...
    int errorLoc = -1;
    Token errorToken = new Token(LexicalAnalyzer.Type.IDENTIFIER, "error", 0);
    ...
    public void reset() {
        ...
        errorLoc = -1;
        errorMessage = "";
        errorToken = new Token(LexicalAnalyzer.Type.IDENTIFIER, "error", 0);
    }
    ...
    public void lexer() {
        if (tokens.size() > 0 && nextTokenPosition < tokens.size()) {
            ...
            if (currentToken.tokenType == LexicalAnalyzer.Type.ERROR) {
    ...
    public String getResult() {
        String result = "";
        String t = tokens.get(tokens.size() - 1).content;
        if (!t.equals("$$") && count>=3) {
            errorLoc = tokens.size() - 1;
            errorToken = tokens.get(tokens.size() - 1);
            errorMessage = "There should be nothing after ending $$";
        ...
        if (errorLoc >= 0) {
            ...
            if (errorToken.tokenType == LexicalAnalyzer.Type.ERROR) {
    ...
    boolean func_rat16f() {
        ...
        if (isToken("$$")) {
            ...
            if (isToken("$$")) {
                ...
                errorLoc = 0;
                errorToken = currentToken;
                errorMessage = "Token '" + errorToken.content + "' at line " + currentToken.lineNumber + ". $$ is expected.";
                return false;
            }
    ...
    boolean func_opt_declaration_list() {
        ...
        if (isToken("{") || currentToken.tokenType == LexicalAnalyzer.Type.IDENTIFIER || isToken("if") || isToken("return") || isToken("print") || isToken("read") || isToken("while")) {
            if (isToken("{")) {
    ...
    boolean func_IDs() {
        ...
        if (currentToken.tokenType == LexicalAnalyzer.Type.IDENTIFIER) {
            ...
            if (isToken(")") || isToken(";") || isToken(":") || isToken("]")) {
                ...
                nextTokenPosition--;
                errorMessage = "invalid declaration";
                errorLoc = nextTokenPosition;
                return false;
            }
        } else {
    ...
    boolean func_return() {
        ...
        if (currentToken.tokenType == LexicalAnalyzer.Type.KEYWORD && isToken("return")) {
    ...
    boolean func_relop() {
        lexer();
        if (isToken("=") || isToken("/=") || isToken(">") || isToken("<") || isToken("=>") || isToken("<=") || isToken("!=")) {
